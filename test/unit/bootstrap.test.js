...
test('fresh bootstrap defaults an absent or empty subscription origin to administration', async () => {
  for (const subscriptionOrigin of [undefined, '']) {
    await temporary(async (parent) => {
      const env = environment(parent);
      env.SUBSCRIPTION_PUBLIC_BASE_URL = subscriptionOrigin;
      env.ADMIN_PUBLIC_HOSTNAME = 'Admin.Example.COM';
      await privateFile(env.TS_AUTH_KEY_FILE, 'tskey-auth-bootstrap-test');
      await privateFile(env.TS_API_KEY_FILE, 'tskey-api-bootstrap-test');
      const result = await bootstrap({
        env,
      ...
      assert.equal(result.status, 'initialized');
      assert.equal(current.state.schemaVersion, 3);
      assert.equal(current.state.gateway.adminPublicHostname, 'admin.example.com');
      assert.equal(current.state.gateway.subscriptionPublicBaseUrl, 'https://admin.example.com');
      assert.equal(current.subscriptionView.gateway.subscriptionPublicHostname, 'admin.example.com');
    });
  }
});
