...
test('server renderer uses only loopback VLESS WebSocket and the ts-out final route', () => {
  ...
  assert.deepEqual(config.experimental, {
    v2ray_api: { listen: '127.0.0.1:19081', stats: { enabled: true, users: ['alice'] } },
  });
  assert.doesNotThrow(() => assertFailClosedConfig(config, lifecycleState()));
  const statsPortClash = { ...lifecycleState(), health: { ...lifecycleState().health, listenPort: 19081 } };
  assert.throws(() => renderSingBoxConfig(statsPortClash), /must differ from 8443 and 19081/u);
});
...
test('server assertion rejects direct fallback and transport weakening', () => {
  ...
  const malformedStats = structuredClone(rendered);
  malformedStats.experimental = { v2ray_api: {} };
  assert.throws(() => assertFailClosedConfig(malformedStats), /unsupported or missing fields/u);
  const publicStats = structuredClone(rendered);
  publicStats.experimental.v2ray_api.listen = '0.0.0.0:19081';
  assert.throws(() => assertFailClosedConfig(publicStats), /traffic statistics/u);
  const untracked = structuredClone(rendered);
  untracked.experimental.v2ray_api.stats.users = [];
  assert.throws(() => assertFailClosedConfig(untracked), /traffic statistics/u);
});
...
test('every exit has isolated DNS and credential-selected routes after the shared address rejects', () => {
  ...
  assert.deepEqual(config.experimental.v2ray_api.stats.users, config.inbounds[0].users.map(({ name }) => name));
...
test('revisions rendered before usage accounting stay valid without the stats block', () => {
  for (const state of [fixtureState(), multipleExitState()]) {
    const { experimental, ...legacy } = renderSingBoxConfig(state);
    assert.equal(experimental.v2ray_api.stats.enabled, true);
    assert.deepEqual(assertFailClosedConfig(legacy, state), legacy);
    assert.deepEqual(assertFailClosedConfig(legacy), legacy);
    const tampered = { ...legacy, experimental: { v2ray_api: { ...experimental.v2ray_api, listen: '0.0.0.0:19081' } } };
    assert.throws(() => assertFailClosedConfig(tampered, state), /config/u);
    assert.throws(() => assertFailClosedConfig({ ...legacy, experimental: {} }, state), /config/u);
  }
});
