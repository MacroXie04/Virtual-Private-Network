...
test('initialized bootstrap is idempotent without rereading settings or rotating secrets', async () => {
  await temporary(async (parent) => {
    ...
    const revisionFiles = ['state.json', 'subscription-view.json', 'manifest.json'];
    const beforeRevision = await Promise.all(revisionFiles.map((name) => readFile(path.join(current.path, name))));
    ...
    const resumed = await repository.readCurrent();
    assert.equal(resumed.id, current.id);
    assert.equal(resumed.state.gateway.subscriptionPublicBaseUrl, 'https://sub.example.com');
    assert.equal(resumed.subscriptionView.gateway.subscriptionPublicHostname, 'sub.example.com');
    assert.deepEqual(
      await Promise.all(revisionFiles.map((name) => readFile(path.join(resumed.path, name)))),
      beforeRevision,
    );
  });
});
