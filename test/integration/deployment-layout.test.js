...
test('container pins supported runtimes and Compose exposes origins only through cloudflared', async () => {
  ...
  assert.match(dockerfile, /-tags "with_gvisor,with_utls,with_tailscale,with_v2ray_api"/u);
  assert.match(dockerfile, /Tags: with_gvisor,with_utls,with_tailscale,with_v2ray_api/u);
  ...
  assert.match(compose, /^\s+SUBSCRIPTION_PUBLIC_BASE_URL: "\$\{SUBSCRIPTION_PUBLIC_BASE_URL:-\}"$/mu);
  assert.match(compose, /^\s+LOCAL_HTTP_ORIGIN: "\$\{LOCAL_HTTP_ORIGIN:-\}"$/mu);
  assert.match(dockerfile, /LOCAL_HTTP_ORIGIN=""/u);
  ...
  assert.match(exampleEnvironment, /^SUBSCRIPTION_PUBLIC_BASE_URL=$/mu);
  assert.match(exampleEnvironment, /^LOCAL_HTTP_ORIGIN=$/mu);
  ...
  const localValidation = entrypoint.indexOf('validateLocalHttpOrigin(process.env.LOCAL_HTTP_ORIGIN);');
  const localInterface = entrypoint.indexOf('127.0.0.1|0.0.0.0)');
  assert.ok(localValidation > 0 && localInterface > localValidation);
  assert.ok(localValidation < entrypoint.indexOf('chown root:root "$DATA_ROOT"'));
  assert.match(entrypoint, /if \[ -n "\$\{LOCAL_HTTP_ORIGIN:-\}" \]; then/u);
  assert.match(entrypoint, /elif \[ "\$\{ADMIN_HOST:-127\.0\.0\.1\}" != 127\.0\.0\.1 \]; then/u);
});
