...
test('bare Cloudflare deployment is pinned, credential-backed, and loopback-only', async () => {
  ...
  assertInstallerMatches(/write_environment_value LOCAL_HTTP_ORIGIN "\$LOCAL_HTTP_ORIGIN"/u);
  ...
  assert.match(admin, /ADMIN_HOST=127\.0\.0\.1 ADMIN_PORT=8081 SUB_PORT=8080/u);
  for (const [unit, identity] of [[subscription, 'vpn-sub'], [admin, 'vpn-admin']]) {
    assert.match(unit, /^EnvironmentFile=\/etc\/vpn-gateway\/admin\.env$/mu);
    assert.match(unit, new RegExp(`^User=${identity}$`, 'mu'));
    assert.match(unit, new RegExp(`^Group=${identity}$`, 'mu'));
    assert.doesNotMatch(unit, /^SupplementaryGroups=/mu);
    assert.doesNotMatch(unit, /^EnvironmentFile=.*(?:controller|runtime|tailscale-api)\.env$/mu);
