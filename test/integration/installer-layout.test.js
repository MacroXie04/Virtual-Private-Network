...
test('supported Node policy and bare-metal ES-module layout stay aligned', async () => {
  ...
  const adminEnvironment = installerFunction('create_admin_environment');
  assert.deepEqual(
    [...adminEnvironment.matchAll(/write_environment_value ([A-Z_]+)/gu)].map((match) => match[1]),
    ['ADMIN_PUBLIC_HOSTNAME', 'LOCAL_HTTP_ORIGIN'],
    'both HTTP services receive only the public hostname and explicit local access setting',
  );
  assert.match(adminEnvironment, /\} >"\$temporary_file"/u);
  assert.match(adminEnvironment, /chmod 0600 "\$temporary_file"/u);
  assert.match(adminEnvironment, /chown root:root "\$temporary_file"/u);
  assert.match(adminEnvironment, /mv -f -- "\$temporary_file" "\$ADMIN_ENV"/u);
...
test('installer defaults only fresh subscription origins and preserves explicit local mode selection', () => {
  const collect = installerFunction('collect_cloudflare_ingress_settings');
  assert.match(collect, /prompt_optional SUBSCRIPTION_PUBLIC_BASE_URL .*"https:\/\/\$ADMIN_PUBLIC_HOSTNAME" no/u);
  const adminPrompt = collect.indexOf('prompt_required ADMIN_PUBLIC_HOSTNAME');
  assert.ok(adminPrompt > 0 && adminPrompt < collect.indexOf('prompt_optional SUBSCRIPTION_PUBLIC_BASE_URL'));
  assert.match(installerFunction('create_fresh_controller_environment'), /collect_cloudflare_ingress_settings yes/u);
  assert.doesNotMatch(installerFunction('create_existing_controller_environment'), /collect_cloudflare_ingress_settings|https:\/\//u);
  assertInstallerMatches(/require_matching_canonical_setting SUBSCRIPTION_PUBLIC_BASE_URL "\$supplied_subscription_url" "\$SUBSCRIPTION_PUBLIC_BASE_URL"/u);
  const localOrigin = installerFunction('resolve_local_http_origin');
  assert.match(localOrigin, /\[\[ -z "\$\{LOCAL_HTTP_ORIGIN\+x\}" \]\]/u);
  assert.match(localOrigin, /read_generated_environment_value "\$ADMIN_ENV" LOCAL_HTTP_ORIGIN yes/u);
  assert.match(localOrigin, /validateLocalHttpOrigin\(process\.env\.LOCAL_HTTP_ORIGIN\)/u);
  const localResolution = installerPosition('resolve_local_http_origin\n');
  assert.ok(localResolution > 0 && localResolution < installerPosition('trap restore_previous_deployment_on_failure EXIT'));
});
