...
async function inspectState(root, surface) {
  if (surface === 'docker') {
    ...
    const stateGuard = entrypoint.indexOf('# Check supported data before creating a lock or normalizing any ownership.');
    assert.ok(stateGuard > 0);
    const start = entrypoint.indexOf(marker, stateGuard) + marker.length;
    const end = entrypoint.indexOf("\n'", start);
    assert.ok(start > stateGuard && end > start);
    ...
    assert.match(script, /await assertSupportedDataDirectory/u);
