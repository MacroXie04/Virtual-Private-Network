...
prompt_required() {
  ...
  if [[ -z "$value" ]]; then
    [[ -t 0 ]] || die "$variable_name must be set for a non-interactive first installation."
    if [[ "$secret" == yes ]]; then
...
[[ $EUID -eq 0 ]] || die "Run this installer as root (for example: sudo bash server/install.sh)."
...
for command_name in bash chmod chown cmp cp cut env find flock getent grep groupadd id install ln mktemp mv node passwd readlink rm rmdir sha256sum sleep stat sync systemctl systemd-notify useradd; do
...
readonly -a REQUIRED_SOURCE_FILES=(
  ...
  src/subscription-server.js
  src/admin-server.js
  src/healthcheck.js
  server/sing-box-wrapper.sh
  server/vpn-gateway.target
  server/vpn-gateway-controller.service
  server/vpn-gateway-sing-box.service
  server/vpn-gateway-subscription.service
  server/vpn-gateway-admin.service
)
...
bash -n "$REPO_DIR/server/install.sh"
...
inspect_legacy_deployment() {
  ...
  local migration_module="${2:-$REPO_DIR/src/migrate-v1.js}"
...
run_legacy_bootstrap() {
  ...
  local bootstrap_module="${4:-$REPO_DIR/src/bootstrap.js}"
  local -a isolated_environment=(
    env -i
    "PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
    "NODE_ENV=production"
...
if [[ "$INSTALL_MODE" == migrate ]]; then
  PUBLIC_BASE_URL="${PUBLIC_BASE_URL:-}"
  prompt_optional PUBLIC_BASE_URL "Externally visible HTTPS base URL (optional)" "" no
...
readonly EXPECTED_RUNTIME_UID EXPECTED_RUNTIME_GID EXPECTED_SUB_UID EXPECTED_SUB_GID EXPECTED_ADMIN_UID EXPECTED_ADMIN_GID
...
hold_upgrade_services_disabled() {
  local target_state singbox_state
  ...
  target_state="$(query_upgrade_enablement vpn-gateway.target no)"
  [[ "$target_state" == disabled ]] \
    || die "vpn-gateway.target remained enabled while preparing the upgrade."
  singbox_state="$(query_upgrade_enablement vpn-gateway-sing-box.service yes)"
...
quiesce_upgrade_services_for_rollback() {
  ...
  systemctl stop \
    ...
    vpn-gateway-subscription.service \
    vpn-gateway-admin.service \
    || die "Could not stop every VPN gateway unit; rollback state was not touched."
  for service_name in \
    vpn-gateway.target \
    vpn-gateway-controller.service \
    vpn-gateway-sing-box.service \
    ...
    [[ "$active_state" == inactive || "$active_state" == failed ]] \
      || die "$service_name remained active after stop; rollback state was not touched."
  done
}
...
restore_previous_deployment_on_failure() {
  ...
  if [[ $status -ne 0 && "$UPGRADE_STOPPED" == yes && "$DEPLOYMENT_HANDOFF_COMPLETE" != yes ]]; then
    echo "==> Upgrade did not become ready; restoring the previous v2 deployment from $UPGRADE_BACKUP" >&2
    if path_is_present "$UPGRADE_ROLLBACK_JOURNAL"; then
      rollback_active_upgrade_transaction
    ...
    for service_name in \
      ...
      vpn-gateway-subscription.service \
      vpn-gateway-admin.service; do
      stop_unit_if_active_strict "$service_name"
    done
    restore_legacy_service_state \
...
if [[ "$INSTALL_MODE" == existing ]]; then
  ...
  echo "==> Publishing restart intent before disabling or stopping the current v2 deployment"
  prepare_upgrade_restart_journal
  UPGRADE_STOPPED=yes
  hold_upgrade_services_disabled
  echo "==> Stopping the current v2 deployment after saving rollback files in $UPGRADE_BACKUP"
  quiesce_upgrade_services_for_rollback
  normalize_upgrade_repository_ownership
...
validate_migration_marker_file() {
  ...
  [[ "$(stat -c '%u:%a' "$marker_file")" == 0:600 ]] \
    || die "Migration marker file $marker_file must be owned by root with mode 0600."
}
...
cleanup_committed_migration_work() {
  ...
  for marker_name in \
    ...
    state-copied state-published committed; do
...
if [[ "$INSTALL_MODE" == migrate ]]; then
  if [[ "$MIGRATION_RESUME" == yes ]]; then
    ...
    for marker_name in sub-active sub-enabled sing-box-active sing-box-enabled state-copied state-published; do
      if path_is_present "$MIGRATION_MARKER/$marker_name"; then
        validate_migration_marker_file "$MIGRATION_MARKER/$marker_name"
      fi
    done
...
install -o root -g root -m 0755 "$REPO_DIR/server/sing-box-wrapper.sh" "$INSTALL_ROOT/bin/sing-box-wrapper"
...
create_fresh_controller_environment() {
  ...
  prompt_required VPS_HOST "Public IP address or DNS name for client links" no
  prompt_required SERVER_NAME "REALITY camouflage DNS name" no
  prompt_optional NODE_NAME "Gateway node name" "vps-reality" no
  prompt_optional PUBLIC_BASE_URL "Externally visible HTTPS base URL (optional)" "" no
  ...
  chmod 0600 "$temporary_file"
  {
    ...
    write_environment_value VPS_HOST "$VPS_HOST"
    write_environment_value SERVER_NAME "$SERVER_NAME"
    write_environment_value NODE_NAME "$NODE_NAME"
    write_environment_value PUBLIC_BASE_URL "$PUBLIC_BASE_URL"
  } >"$temporary_file"
  chown root:root "$temporary_file"
...
preserve_legacy_credentials() {
  env -i \
    "PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" \
    "BOOTSTRAP_MODULE=$INSTALL_ROOT/src/bootstrap.js" \
    "MIGRATION_MODULE=$INSTALL_ROOT/src/migrate-v1.js" \
    "LEGACY_ENV_FILE=$LEGACY_ENV_FILE" \
    "LEGACY_CONFIG_FILE=$LEGACY_CONFIG_FILE" \
...
case "$INSTALL_MODE" in
  fresh)
    ...
    if path_is_present "$CONTROLLER_ENV"; then
      ...
      for required_key in TS_AUTH_KEY_FILE EXIT_NODE VPS_HOST SERVER_NAME NODE_NAME PUBLIC_BASE_URL; do
  ...
  existing)
    if path_is_present "$CONTROLLER_ENV"; then
      ...
      install -o root -g root -m 0600 /dev/null "$CONTROLLER_ENV"
    fi
    echo "==> Preserving existing controller environment and persistent state"
    ;;
esac
...
create_admin_environment() {
  local temporary_file
  ADMIN_ALLOWED_HOSTS="${ADMIN_ALLOWED_HOSTS:-admin.vpn.invalid:8444}"
  ADMIN_ALLOWED_ORIGINS="${ADMIN_ALLOWED_ORIGINS:-https://admin.vpn.invalid:8444}"

  temporary_file="$(mktemp "$ENV_ROOT/.admin.env.XXXXXX")"
  chmod 0600 "$temporary_file"
  {
    write_environment_value ADMIN_ALLOWED_HOSTS "$ADMIN_ALLOWED_HOSTS"
    write_environment_value ADMIN_ALLOWED_ORIGINS "$ADMIN_ALLOWED_ORIGINS"
  } >"$temporary_file"
  chown root:root "$temporary_file"
  mv -f -- "$temporary_file" "$ADMIN_ENV"
}

if [[ -e "$ADMIN_ENV" || -L "$ADMIN_ENV" ]]; then
  [[ -f "$ADMIN_ENV" && ! -L "$ADMIN_ENV" ]] \
    || die "$ADMIN_ENV must be a regular file, not a symlink."
  for required_key in ADMIN_ALLOWED_HOSTS ADMIN_ALLOWED_ORIGINS; do
    [[ "$(grep -c "^${required_key}=" "$ADMIN_ENV")" == 1 ]] \
      || die "$ADMIN_ENV must contain exactly one $required_key assignment; repair it before rerunning."
  done
...
if [[ "$INSTALL_MODE" == migrate ]]; then
  ...
  if [[ "$COPY_LEGACY_STATE" == yes && "$STATE_COPY_ALREADY_COMPLETE" != yes ]]; then
    validate_regular_tree "$LEGACY_SOURCE_STATE" "Legacy Tailscale state"
  ...
  grep -Fq MIGRATION_STATE_DIR "$INSTALL_ROOT/src/migrate-v1.js" \
  ...
  inspect_legacy_deployment "$STATE_ROOT/tailscale" "$INSTALL_ROOT/src/migrate-v1.js"
  if path_is_present "$STATE_ROOT/current" || path_is_present "$STATE_ROOT/runtime"; then
    echo "==> Recovering the already published legacy migration"
    run_legacy_bootstrap 1 existing,recovered "$STATE_ROOT/tailscale" "$INSTALL_ROOT/src/bootstrap.js"
  else
    echo "==> Applying the explicitly approved legacy migration"
    run_legacy_bootstrap 1 migrated "$STATE_ROOT/tailscale" "$INSTALL_ROOT/src/bootstrap.js"
  fi
  ...
  env -i \
    ...
    "$NODE_BIN" --input-type=module --eval '
      import { RevisionRepository } from "/opt/vpn-gateway/src/repository.js";
      ...
      if (!current || !runtime || current.id !== runtime.id || current.manifest.operation !== "migrate-v1"
          || current.state.tailscale.stateDirectory !== process.env.EXPECTED_STATE_DIRECTORY) {
        throw new Error("migrated state does not use the copied Tailscale state directory");
      }
    '
...
for unit_file in "${UNIT_FILES[@]}"; do
  install -o root -g root -m 0644 "$REPO_DIR/server/$unit_file" "$SYSTEMD_ROOT/$unit_file"
done
...
for ((attempt = 1; attempt <= 45; attempt += 1)); do
  if "$NODE_BIN" "$INSTALL_ROOT/src/healthcheck.js" >/dev/null 2>&1; then
    gateway_ready=yes
    break
  fi
...
if [[ -n "$UPGRADE_BACKUP" ]]; then
  echo "==> Previous v2 deployment retained for manual rollback at $UPGRADE_BACKUP"
fi
...
Deployment complete.
  ...
  VPN data plane:       TCP 443
  Subscription service: http://subscriptions.localhost:8080
  Admin HTTP transport: 127.0.0.1:8081 (never browse or direct-forward)
  Administration URL:   https://admin.vpn.invalid:8444 (trusted TLS frontend)

The web services intentionally listen only on loopback. The administration
backend is HTTP transport for the trusted local TLS frontend only. Never browse
it directly, forward its TCP port to a browser, or bypass TLS certificate checks.
