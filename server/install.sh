...
readonly -a REQUIRED_SOURCE_FILES=(
  ...
  server/vpn-gateway-tunnel.service
)
...
validate_cloudflare_ingress_settings() {
  ...
  normalized_output="$(
    env -i \
      "PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" \
      "VALIDATION_MODULE=$REPO_DIR/src/validation.js" \
...
inspect_current_state_for_installer() {
  env -i \
    "PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" \
    "DATA_DIR=$STATE_ROOT" \
    "STATE_SCHEMA_MODULE=$REPO_DIR/src/state-schema.js" \
    "$NODE_BIN" --input-type=module --eval '
      import { constants } from "node:fs";
...
systemd-analyze verify "${UNIT_FILES[@]/#/$REPO_DIR/server/}" \
...
if [[ "$INSTALL_MODE" == migrate ]]; then
  ...
  if path_is_present "$STATE_ROOT/current" || path_is_present "$STATE_ROOT/runtime"; then
    ...
    run_legacy_bootstrap 1 migrated,recovered "$STATE_ROOT/tailscale" "$INSTALL_ROOT/src/bootstrap.js"
  fi
  ...
  env -i \
    ...
    "$NODE_BIN" --input-type=module --eval '
      ...
      import { assertLegacyV1MigrationLineage } from "/opt/vpn-gateway/src/migrate-v1.js";
