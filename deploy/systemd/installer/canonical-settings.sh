    ...
    env -i \
      ...
      "$NODE_BIN" --input-type=module --eval '
        ...
        if ([gateway.vpnPublicHostname, subscriptionHost, gateway.adminPublicHostname].includes(healthHost)) {
          throw new Error("EGRESS_HEALTH_HOST must be independent of every Tunnel hostname");
        }
...
collect_cloudflare_ingress_settings() {
  ...
  prompt_optional SUBSCRIPTION_PUBLIC_BASE_URL "Public subscription HTTPS origin" "https://$ADMIN_PUBLIC_HOSTNAME" no
  if [[ -z "$WS_PATH" ]]; then
  ...
  env -i \
    ...
    "$NODE_BIN" --input-type=module --eval '
      ...
      try {
        ...
        if (result === undefined && process.argv[3] === "yes") result = "";
        if (result === undefined || /[\r\n\0]/u.test(result)) throw new Error("environment setting is missing or unsafe");
        process.stdout.write(result);
      } finally {
        await handle?.close().catch(() => {});
      }
    ' "$environment_path" "$setting_name" "$allow_missing"
}

resolve_local_http_origin() {
  if [[ -z "${LOCAL_HTTP_ORIGIN+x}" ]] && path_is_present "$ADMIN_ENV"; then
    LOCAL_HTTP_ORIGIN="$(read_generated_environment_value "$ADMIN_ENV" LOCAL_HTTP_ORIGIN yes)" \
      || die "$ADMIN_ENV does not contain safe local HTTP settings."
  fi
  LOCAL_HTTP_ORIGIN="${LOCAL_HTTP_ORIGIN:-}"
  [[ -n "$LOCAL_HTTP_ORIGIN" ]] || return 0
  env -i \
    "PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" \
    "VALIDATION_MODULE=$REPO_DIR/src/http/admin/site.js" \
    "LOCAL_HTTP_ORIGIN=$LOCAL_HTTP_ORIGIN" \
    "$NODE_BIN" --input-type=module --eval '
      import { pathToFileURL } from "node:url";
      const { validateLocalHttpOrigin } = await import(pathToFileURL(process.env.VALIDATION_MODULE).href);
      validateLocalHttpOrigin(process.env.LOCAL_HTTP_ORIGIN);
    ' || die "LOCAL_HTTP_ORIGIN must be a canonical HTTP origin on localhost, 127.0.0.1 or [::1]."
}
