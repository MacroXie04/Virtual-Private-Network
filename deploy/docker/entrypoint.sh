...
if [ -n "${LOCAL_HTTP_ORIGIN:-}" ]; then
  node --input-type=module --eval '
    import { validateLocalHttpOrigin } from "/app/src/http/admin/site.js";
    validateLocalHttpOrigin(process.env.LOCAL_HTTP_ORIGIN);
  '
  case "${ADMIN_HOST:-127.0.0.1}" in
    127.0.0.1|0.0.0.0) ;;
    *) echo "Local administration requires the fixed container listener." >&2; exit 1 ;;
  esac
elif [ "${ADMIN_HOST:-127.0.0.1}" != 127.0.0.1 ]; then
  echo "Public administration must use its fixed loopback-only Tunnel origin." >&2
  exit 1
fi
...
for file in /app/src/state/bootstrap.js /app/src/control/controller-server.js /app/src/http/subscription-server.js /app/src/http/admin-server.js /app/src/runtime/healthcheck.js; do
  if [ ! -r "$file" ]; then
...
node /app/src/state/bootstrap.js
exec node /app/src/control/controller-server.js
