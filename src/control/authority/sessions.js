...
export class ControllerSessions {
  ...
  issue(credentialVersion = 1, subject = null) {
    if (subject !== null && (typeof subject !== 'string' || subject.length < 1 || subject.length > 64)) {
      throw new TypeError('session subject is invalid');
    }
    this.prune();
  ...
  /** End every session bound to one subject, optionally sparing the caller's own. */
  destroySubject(subject, exceptKey = null) {
    let removed = 0;
    for (const [key, session] of this.sessions) {
      if (session.subject === subject && key !== exceptKey) {
        this.sessions.delete(key);
        removed += 1;
      }
    }
