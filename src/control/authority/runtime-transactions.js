...
/** Counters restart with sing-box, so fold the final deltas before every restart. */
async function restartRuntime(controller) {
  await controller.sampleUsage?.();
  await controller.runtime.restart();
  controller.usage?.markRestarted();
}

export async function recover(controller) {
  await assertSupportedDataDirectory(controller.dataDir);
  ...
  if (runtimeId && runtimeId !== current.id) {
    await controller.repository.assertSupportedRevisionSchema(runtimeId);
  }
  ...
  try {
    ...
    await restartRuntime(controller);
...
export async function retireBootstrapCredentials(controller, current = null) {
  ...
  for (const revision of revisions) {
    ...
    const obsolete = await controller.repository.readRevision(revision.id);
    if (hasEnrollmentCredentials(obsolete.state)) {
...
export async function transact(controller, candidateState, {
  ...
  // The restart hint holds only while the rendered data plane is unchanged; a
  // revision inherited from an older release may differ (for example by gaining
  // the stats block), and sing-box must then run what the runtime pointer names.
  const [previousConfig, candidateConfig] = await Promise.all([
    readFile(path.join(previous.path, 'sing-box.json')),
    readFile(path.join(candidate.path, 'sing-box.json')),
  ]);
  const mustRestart = restart || !previousConfig.equals(candidateConfig);
  controller.ready = false;
  try {
    await controller.repository.activateRuntime(candidate.id);
    controller.runtime.health = buildRuntimeHealth(candidateState);
    if (mustRestart) await restartRuntime(controller);
