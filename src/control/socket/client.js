...
import { ControlError, sendControlRequest } from './client-transport.js';
...
export function createControlClient({
  ...
  return Object.freeze({
    ...
    renameUser: (sessionId, csrf, expectedRevision, userId, displayName) => request('user.rename', {
      sessionId, csrf, expectedRevision, userId, displayName,
    }),
    ...
    // Re-running a lost password reset or token rotation simply issues a fresh secret,
    // so neither carries a replay id nor retries transport.
    resetUserPassword: (sessionId, csrf, expectedRevision, userId) => request('user.resetPassword', {
      sessionId, csrf, expectedRevision, userId,
    }),
    accountLogin: (displayName, password) => request('account.login', { displayName, password }),
    accountCheck: (sessionId) => request('account.check', { sessionId }),
    accountLogout: (sessionId, csrf) => request('account.logout', { sessionId, csrf }),
    accountSnapshot: (sessionId) => request('account.snapshot', { sessionId }),
    accountExport: (sessionId, format) => request('account.export', { sessionId, format }),
    accountRotateToken: (sessionId, csrf) => request('account.rotateToken', { sessionId, csrf }),
    accountChangePassword: (sessionId, csrf, currentPassword, newPassword) => request('account.changePassword', {
      sessionId, csrf, currentPassword, newPassword,
    }),
