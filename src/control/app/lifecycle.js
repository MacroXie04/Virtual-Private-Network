...
export function createApplicationLifecycle({
  ...
  const cleanup = () => {
    ...
    cleanupPromise = (async () => {
      ...
      authority.accountSessions?.destroyAll?.();
      await web?.stop().catch(() => { exitCode = 1; });
      // Capture the last traffic deltas before the data plane stops.
      await authority.sampleUsage?.();
