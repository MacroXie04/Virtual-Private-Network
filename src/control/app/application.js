...
import { UsageTracker } from '../authority/usage.js';
...
export async function createControllerApplication({
  ...
  const usageTracker = usage ?? new UsageTracker({ path: path.join(dataDir, 'usage.json') });
  await usageTracker.load().catch(() => {
    process.stderr.write('VPN gateway usage history could not be loaded; totals restart from zero.\n');
  });
