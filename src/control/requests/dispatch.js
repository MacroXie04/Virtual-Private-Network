import { getUser } from '../../core/users/lifecycle.js';
import { renderVlessLink } from '../../core/subscriptions/clients.js';
import { ControllerError, exactObject, stringField, operationError } from './contract.js';
import { hasEnrollmentCredentials } from '../authority/state-views.js';
import { dispatchMutation } from './mutations.js';
import { dispatchAccount } from './accounts.js';

export async function dispatch(controller, request) {
  try {
    exactObject(request, ['id', 'op', 'secret', 'sessionId', 'csrf', 'expectedRevision', 'displayName', 'userId', 'status', 'confirmName', 'deviceId', 'exitId', 'url', 'password', 'currentPassword', 'newPassword', 'format'], ['id', 'op']);
    ...
    // The end-user realm never enters the administrator replay or session path.
    if (op.startsWith('account.')) return await dispatchAccount(controller, request);
