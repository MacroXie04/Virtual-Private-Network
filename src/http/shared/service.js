...
import { HttpError, MAX_HEADER_BYTES } from './input.js';
...
export function sendResponse(req, res, status, body = '', headers = {}) {
  ...
  const merged = {
    ...headers,
    ...SECURITY_HEADERS,
    // Same-origin HTML forms need a non-null Origin; retain the stricter default elsewhere.
    'referrer-policy': headers['referrer-policy'] === 'same-origin' ? 'same-origin' : SECURITY_HEADERS['referrer-policy'],
...
export function tooManyRequests(req, res, rate) {
