...
import { NOFOLLOW, DIRECTORY, SERVICE_UID, PRIVATE_GID, RepositoryError } from './policy.js';
...
export async function inspectOwnedDirectory(directoryPath, {
  ownerUid = SERVICE_UID,
  ownerGid = PRIVATE_GID,
} = {}) {
  const before = await safeLstat(directoryPath);
  if (before === null) return false;
  let handle;
  try {
    handle = await open(directoryPath, fsConstants.O_RDONLY | DIRECTORY | NOFOLLOW);
    const stat = await handle.stat();
    if (!stat.isDirectory() || stat.uid !== ownerUid || stat.gid !== ownerGid
      || (stat.mode & 0o022) !== 0 || stat.dev !== before.dev || stat.ino !== before.ino) {
      throw new RepositoryError('UNSAFE_PATH', 'repository directory ownership or permissions are unsafe');
    }
  } catch (error) {
    if (error instanceof RepositoryError) throw error;
    throw new RepositoryError('UNSAFE_PATH', 'repository directory could not be inspected safely');
  } finally {
