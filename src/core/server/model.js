import { deriveExitHealthPassword, deriveExitUuid } from '../identity/exit-profiles.js';
import { HEALTH_USERNAME, STATS_LISTEN_PORT, VLESS_LISTEN_HOST, VLESS_LISTEN_PORT } from '../model/policy.js';
...
export function renderServerConfig(state) {
  ...
  // Per-user traffic counters for the administration UI. The listener is
  // loopback-only and tracks exactly the public inbound credentials.
  config.experimental = {
    v2ray_api: {
      listen: `${VLESS_LISTEN_HOST}:${STATS_LISTEN_PORT}`,
      stats: { enabled: true, users: activeUsers.map((user) => user.name) },
    },
  };
  ...
  config.experimental.v2ray_api.stats.users = config.inbounds[0].users.map((user) => user.name);
  return config;
}
