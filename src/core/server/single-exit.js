import { HEALTH_PASSWORD_BYTES, HEALTH_USERNAME, STATS_LISTEN_PORT, VLESS_LISTEN_HOST, VLESS_LISTEN_PORT } from '../model/policy.js';
...
export function assertSingleExitConfig(value, expectedState = null) {
  ...
  // Revisions written before usage accounting carry no stats block; they stay
  // valid as stored and gain the block on their next transaction.
  const hasStats = Object.hasOwn(value, 'experimental');
  requireExactKeys(value, ['log', 'dns', 'inbounds', 'endpoints', 'route', ...(hasStats ? ['experimental'] : [])], 'config');
  ...
  if (hasStats) {
    requireExactKeys(value.experimental, ['v2ray_api'], 'config.experimental');
    requireExactKeys(value.experimental.v2ray_api, ['listen', 'stats'], 'config.experimental.v2ray_api');
    requireExactKeys(value.experimental.v2ray_api.stats, ['enabled', 'users'], 'config.experimental.v2ray_api.stats');
    if (value.experimental.v2ray_api.listen !== `${VLESS_LISTEN_HOST}:${STATS_LISTEN_PORT}`
      || value.experimental.v2ray_api.stats.enabled !== true
      || !sameArray(value.experimental.v2ray_api.stats.users, publicInbound.users.map((user) => user.name))) {
      fail('config.experimental', 'must expose loopback traffic statistics for exactly the public users');
    }
  }
