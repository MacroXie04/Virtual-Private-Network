...
import { HEALTH_PASSWORD_BYTES, HEALTH_USERNAME, MAX_USERS, STATS_LISTEN_PORT, VLESS_LISTEN_PORT } from '../model/policy.js';
...
function assertMultipleExitConfig(value, expectedState) {
  ...
  if (listenPort === VLESS_LISTEN_PORT || listenPort === STATS_LISTEN_PORT) fail('config.inbounds', 'listen ports must differ');
  ...
  // Revisions written before usage accounting carry no stats block.
  if (!Object.hasOwn(value, 'experimental')) delete expected.experimental;
