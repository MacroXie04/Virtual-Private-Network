...
export function validatePublicIngressSettings(value, path = 'gateway') {
  ...
  if (vpnPublicHostname === subscriptionHostname || vpnPublicHostname === adminPublicHostname) {
    throw new ValidationError(path, 'VPN and HTTP service hostnames must be distinct');
  }
  return {
    vpnPublicHostname,
    subscriptionPublicBaseUrl,
