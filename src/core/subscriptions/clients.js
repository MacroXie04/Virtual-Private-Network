...
/** Named links in client-profile order: the lone default keeps the user's name. */
export function renderConnections(value, selector) {
  return resolveConnections(value, selector).map((config) => ({ name: config.name, link: connectionLink(config) }));
}
