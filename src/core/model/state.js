...
import { STATE_SCHEMA_VERSION, MAX_USERS, MAX_USER_RECORDS, STATS_LISTEN_PORT, VLESS_LISTEN_PORT } from './policy.js';
...
function validateStateWithOptions(value) {
  ...
  if ([VLESS_LISTEN_PORT, STATS_LISTEN_PORT].includes(normalized.health.listenPort)) {
    throw new ValidationError('state.health.listenPort', `must differ from ${VLESS_LISTEN_PORT} and ${STATS_LISTEN_PORT}`);
  }
