...
/** Resolve a sign-in name to its non-revoked user; anything that cannot be a name is null. */
export function findUserBySignInName(value, name) {
  const state = validateState(value);
  let key;
  try {
    key = normalizeDisplayName(name, 'name').toLowerCase();
  } catch (error) {
    if (error instanceof ValidationError) return null;
    throw error;
  }
  return state.users.find((user) => (
    user.status !== 'revoked' && user.displayName.toLowerCase() === key
  )) ?? null;
}
...
export function createUser(value, input, {
  ...
  // The portal password arrives already derived; scrypt never runs inside a mutation.
  const password = input.password === undefined
    ? null
    : validateScryptRecord(input.password, 'input.password');
  const user = {
    id,
    displayName,
    uuid,
...
export function revokeUser(value, id, { now } = {}) {
  ...
  const current = state.users[index];
  assertMutable(current);
  // Tombstones keep no verifier: a revoked record can never authenticate again.
  const { password: _retired, ...retained } = current;
  const user = {
    ...retained,
    status: 'revoked',
    updatedAt: timestamp,
