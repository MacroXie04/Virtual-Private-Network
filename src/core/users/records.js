...
export function validateUser(value, path) {
  const user = expectExactKeys(value, [
    ...
    'disabledAt',
    'revokedAt',
    // Portal password: optional so revisions written before accounts existed stay valid.
  ], path, { optional: ['password'] });
  ...
  if (status === 'revoked' && Object.hasOwn(user, 'password')) {
    throw new ValidationError(path, 'revoked users cannot retain password material');
  }
  return {
    ...
    ...(Object.hasOwn(user, 'password')
      ? { password: validateScryptRecord(user.password, `${path}.password`) }
      : {}),
  };
}
