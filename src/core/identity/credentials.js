...
/** UTF-8 byte bounds for every scrypt-protected password; the minimum applies when one is set. */
export const PASSWORD_BYTES = Object.freeze({ min: 12, max: 1024 });
...
function validatePassword(value, path = 'password', { requireMinimum = false } = {}) {
  const password = expectString(value, path, { min: 1, max: PASSWORD_BYTES.max, controls: true });
  const bytes = Buffer.byteLength(password, 'utf8');
  if (bytes > PASSWORD_BYTES.max) throw new ValidationError(path, `must be at most ${PASSWORD_BYTES.max} UTF-8 bytes`);
  if (requireMinimum && bytes < PASSWORD_BYTES.min) {
    throw new ValidationError(path, `must be at least ${PASSWORD_BYTES.min} UTF-8 bytes`);
  }
  return password;
}
...
/** 32 symbols without the look-alikes 0/1/8/l, so a shown-once password can be typed. */
export const TYPEABLE_SYMBOLS = 'abcdefghijkmnopqrstuvwxyz2345679';

/** Generated portal password: four groups of five symbols (100 bits, 23 bytes). */
export function createUserPassword({ randomBytesImpl = randomBytes } = {}) {
  const bytes = randomBytesImpl(20);
  if (!Buffer.isBuffer(bytes) || bytes.length !== 20) {
    throw new Error('secure random source returned an invalid password');
  }
  const symbols = [...bytes].map((byte) => TYPEABLE_SYMBOLS[byte & 31]);
