...
For public deployments, open `https://ADMIN_PUBLIC_HOSTNAME`: `/login` provides login; `/`, `/exit-nodes`, and `/users` provide the overview, exit-node, and user pages; and `/s/…` serves subscriptions on the same origin. Cloudflare Access is not a replacement for the application administrator secret. Keep interactive Access and browser challenges off the subscription paths. Direct HTTP access is available only through the explicit [local configuration](deployment.md#local-access-without-cloudflare).
