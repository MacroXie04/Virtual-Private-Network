...
src/
  core/
    ...
    users/        records.js, mutation-context.js, lifecycle.js, credential-rotation.js
                  (user records carry an optional portal-password scrypt record)
  ...
  control/
    ...
    authority/    controller.js, sessions.js, accounts.js, state-views.js,
                  operational-files.js, runtime-transactions.js, usage.js
    requests/     contract.js, dispatch.js, mutations.js, users.js, gateway.js, accounts.js
  ...
  http/
    ...
    admin/        application.js, auth.js, request.js, routes.js, user-routes.js,
                  site.js, subscriptions.js
      pages/      document.js, layout.js, styles.js, access.js, dashboard.js,
                  overview.js, users.js, exits.js
    account/      access.js, routes.js, pages.js, portal-page.js
    subscription/ application.js, data.js, request.js, response.js
  runtime/
    ...
    stats/        client.js, grpc.js, protobuf.js
  state/
...
- Put control startup in `src/control/app/`, serialized authority in `authority/`, authorized request handling in `requests/`, and the Unix-socket transport in `socket/`. Administrator pages belong in `src/http/admin/pages/`; the end-user portal (its sign-in, routes and pages) belongs in `src/http/account/` and reuses the page primitives from `admin/pages/`; admin and subscription handlers belong in their respective HTTP areas, with shared transport helpers in `http/shared/`.
...
| Controller authority | `control/authority/controller.js` owns sessions, serialization and authority, delegating to the other modules in `authority/`. `accounts.js` owns both realms' sign-in and the end-user session realm (`controller.accountSessions`, a second bounded store whose ids are meaningless to the administrator store). `runtime-transactions.js` owns runtime commit/rollback, recovery and credential retirement. `usage.js` folds sing-box's per-user traffic counters into `usage.json`, sampled by the watchdog, before every data-plane restart and at shutdown. |
| Control requests | `control/requests/contract.js` owns operation constraints. `dispatch.js` separates reads from mutations and routes every `account.*` operation to `accounts.js`, which resolves the acting user from the session subject and never touches the administrator store; `mutations.js` applies replay, CSRF and revision checks before dispatching to `users.js` or `gateway.js`. |
...
| HTTP site | `http/admin/application.js` composes request checks, authentication and routes from `request.js`, `auth.js` and `routes.js`; `user-routes.js` owns the user mutation forms and `pages/` owns page rendering (`pages/access.js` renders the pages outside the dashboard shell: the public home page, administrator sign-in and the error page both realms use for controller failures). `site.js` validates public and optional local site access. `subscriptions.js` forwards subscription requests to the fixed internal worker without reading projection files. `http/account/` serves the end-user portal under `/account` on the same origin with its own cookies, limits and error handling: `access.js` owns sign-in and the session gate, `routes.js` the authenticated routes, `pages.js` and `portal-page.js` the rendering. |
| Subscription worker and shared HTTP | `http/subscription/application.js` serves projections read through `data.js`; `request.js` and `response.js` own its request and response contracts. It accepts the canonical administration hostname and the stored subscription hostname. `http/shared/` owns transport, input and limiter behavior. The two HTTP `*-server.js` files start the services. |
| Runtime and probes | `runtime/sing-box/` owns configuration checks and supervised or systemd process lifecycles. `runtime/health/readiness.js` coordinates WebSocket and authenticated SOCKS probes; the other health modules own those wire protocols. `runtime/stats/` queries sing-box's loopback V2Ray stats API over HTTP/2 gRPC with a hand-written protobuf codec, which is why the build needs the `with_v2ray_api` tag. |
...
Canonical state uses schema version 3. Reject unsupported state formats without converting or overwriting them. Fresh initialization and recovery of current-format revisions are the supported startup paths. Additive keys stay optional: `tailscale.extraExits` and a user's portal `password` record are allow-listed only when present, so revisions written before they existed remain valid; never make such a key required.

The administration site and subscriptions share a public origin while retaining separate processes and filesystem permissions. New public subscription URLs use the administration hostname; historical URLs remain supported through the stored subscription origin. A fresh bootstrap may default an omitted subscription origin to the administration origin, but existing canonical settings must not be rewritten. VPN WSS uses its own hostname. `LOCAL_HTTP_ORIGIN` enables only explicitly configured loopback HTTP site access; the public deployment keeps its existing Cloudflare-only ingress boundary.
...
For local work on the administration site, `.claude/launch.json` starts `.claude/dev-server.mjs`, which runs the real controller, control socket, administration site, and subscription worker on loopback ports 18081 and 18080 with a fresh temporary state directory. It replaces only the sing-box data plane with a stub that reports every restart and readiness probe as successful, so it exercises the control plane and UI but never carries VPN traffic. The administrator secret defaults to `local-dev-password`, and a seeded "Demo User" account signs in to the portal at `/account/login` with `local-dev-account`.
...
Group tests by behavior: controller authorization, credentials, users, accounts, health and transactions; admin authentication, origin checks, mutations and rate limits; the account portal's sign-in, routes and limits; and bootstrap initialization, recovery and unsupported-state rejection. Share setup through fixtures or helpers without hiding the assertions. HTTP entry-point tests launch the real executables, check responses and verify graceful termination.
