...
| `admin.example.com` | `http://127.0.0.1:8081` — home page, login, dashboard, `/account` portal, and `/s/…` subscriptions |
...
Use [Cloudflare Access](https://developers.cloudflare.com/cloudflare-one/access-controls/applications/http-apps/self-hosted-public-app/) on the site's management paths as an additional control; application administrator authentication, secure cookies, CSRF checks, exact Host/Origin checks, and rate limits remain required in every layout. Configure `/s/*` so subscription clients can fetch it without interactive Access, browser challenges, or redirects, unless every client supplies supported non-interactive credentials. A hostname-wide interactive policy also blocks subscriptions now that they share the site hostname. End users reach their own portal at `/account` and beneath on the same hostname, every page loads `/assets/*`, and the site root `/` is a constant public home page that links to both realms, reads no cookie and never contacts the controller. Write the portal bypass application as the path `/account` without a wildcard: an [application path](https://developers.cloudflare.com/cloudflare-one/access-controls/policies/app-paths/) covers itself and, by inheritance, everything beneath it, whereas `/account/*` does not cover `/account` itself, which is exactly where a completed sign-in and the home page send users. A deployment that created the bypass as `/account/*` following an earlier version of this guide must change it to `/account`; until then, completing a portal sign-in lands on the administrator Access prompt. Choose the layout by whether anonymous visitors should reach the home page:

- **Public home page.** Create the administrator application on the management paths `/login`, `/logout`, `/overview`, `/users`, `/exit-nodes`, `/exit-node` and `/public-base` (each without a wildcard, so everything beneath inherits it) and leave the rest of the hostname without an interactive application, so `/`, `/account`, `/assets/*` and `/s/*` are reachable without Access. The application admits only those paths to the administrator pipeline and answers every other path with 404 before reading a cookie or contacting the controller, so nothing outside the application can reach an administrator route.
- **Home page behind Access.** Keep the administrator application on the whole hostname and add more specific applications for `/s/*`, `/account` and `/assets/*` that bypass interactive Access (the more specific path takes precedence), so every administrator route stays covered without enumerating it. A bypass for exactly `/` cannot be expressed, so anonymous visitors to the root see the Access prompt instead of the home page; hand users the `/account/login` address in this layout. Existing deployments keep this layout on upgrade.

In either layout, consider a Cloudflare rate-limiting rule on `/account/login` in addition to the application's own limits. Do not add a Cache Rule that overrides the origin's `cache-control: no-store` on the site hostname; every page, including the home page, is served uncached. Redact subscription-token paths from all logs.
...
The `/dev/null` value satisfies base-file interpolation only; no Tunnel service is started or given a credential. This local command does not use the public Tunnel launcher. Tailscale enrollment, approved exits and the routed health target are still required. Open `http://127.0.0.1:8081` for the home page; administrators sign in at `/login` and reach the dashboard, subscription paths are served by the same listener, and users sign in to their portal at `http://127.0.0.1:8081/account/login`.
